...
class GPT_Network(tf.keras.Model):
    ...
    p_keep=0.9, p_reg=1.0, var_type="norm_add", **kwargs):
        ...
        rel_pos_shape = [2*self.seq_length+1, self.head_size]
        ...
        self.W_rel_pos_k = tf.Variable(tf.random.normal(
            rel_pos_shape, stddev=0.1), name="rel_pos_embed_k")
        self.W_rel_pos_v = tf.Variable(tf.random.normal(
            rel_pos_shape, stddev=0.1), name="rel_pos_embed_v")
        ...
        # GPT Variables. #
        layer_norm_shp  = [self.n_layers, self.hidden_size]
        attn_wgt_shape  = [
        ...
        self.b_d_bias_i = tf.Variable(tf.zeros(
            layer_norm_shp), name="b_d_bias_i")
        self.b_d_bias_1 = tf.Variable(tf.zeros(
            layer_norm_shp), name="b_d_bias_1")
        self.b_d_bias_2 = tf.Variable(tf.zeros(
            layer_norm_shp), name="b_d_bias_2")
        
        self.b_d_scale_i = tf.Variable(tf.ones(
            layer_norm_shp), name="b_d_scale_i")
        self.b_d_scale_1 = tf.Variable(tf.ones(
            layer_norm_shp), name="b_d_scale_1")
        self.b_d_scale_2 = tf.Variable(tf.ones(
            layer_norm_shp), name="b_d_scale_2")
    ...
    def transformer_decode(
        ...
        rel_pos_reshape_q = [
            batch_size*n_heads, step, self.head_size]
        rel_pos_reshape_v = [
            batch_size, n_heads, step, self.head_size]
        
        # Relative Position Embeddings. #
        rel_pos_index = np.zeros(
            (step, step), dtype=np.int32)
        for ni in range(step):
            rel_pos_index[ni, :] = [
                (step-ni+x) for x in range(step)]
        
        rel_pos_embed_k = tf.nn.embedding_lookup(
            self.W_rel_pos_k, rel_pos_index)
        rel_pos_embed_v = tf.nn.embedding_lookup(
            self.W_rel_pos_v, rel_pos_index)
        rel_pos_embed_k = tf.transpose(
            rel_pos_embed_k, [0, 2, 1])
        del rel_pos_index
        ...
        for m in range(self.n_layers):
            ...
            # For relative position of keys. #
            x_sqt = tf.reshape(x_sq, rel_pos_reshape_q)
            x_sqt = tf.transpose(x_sqt, [1, 0, 2])
            
            x_rel_k = tf.matmul(x_sqt, rel_pos_embed_k)
            x_rel_k = x_rel_k * tf.math.rsqrt(head_size)
            x_rel_k = tf.transpose(x_rel_k, [1, 0, 2])
            x_rel_k = tf.reshape(
                x_rel_k, [batch_size, n_heads, step, step])
            ...
            x_s_alphas = tf.nn.softmax(
                x_s_scores + x_rel_k + attn_mask)
            
            # For the xv projection. #
            x_s_alphas_v = tf.reshape(
                x_s_alphas, [batch_size*n_heads, step, step])
            x_s_alphas_v = tf.transpose(x_s_alphas_v, [1, 0, 2])
            
            x_rel_v = tf.matmul(
                x_s_alphas_v, rel_pos_embed_v)
            x_rel_v = tf.transpose(x_rel_v, [1, 0, 2])
            x_rel_v = tf.reshape(x_rel_v, rel_pos_reshape_v)
            
            x_self_conc  = combine_heads(
                tf.matmul(x_s_alphas, x_sv) + x_rel_v)
