...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(
        self, x, mask, enc_outputs, training=True):
    ...
    def call(self, x, enc_outputs, training=True):
        ...
        for m in range(self.n_layers):
            layer_output = self.dec_layers[m](
                layer_input, input_mask, 
                enc_outputs, training=training)
            layer_input  = layer_output
        return layer_output
...
class Transformer(tf.keras.Model):
    def __init__(
        ...
        enc_seq_length, dec_seq_length, rate=0.1):
        ...
        self.d_ffwd  = d_ffwd
        self.d_model = d_model
        self.enc_seq_len = enc_seq_length
        ...
        self.encoder_model = Encoder(
            ...
            enc_vocab_size, enc_seq_length, rate=rate)
        self.decoder_model = Decoder(
            n_layers, d_model, n_heads, d_ffwd, 
            dec_vocab_size, dec_seq_length, rate=rate)
