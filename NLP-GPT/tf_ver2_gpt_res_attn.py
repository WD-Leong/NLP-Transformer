...
class GPT_Network(tf.keras.Model):
    ...
    def transformer_decode(
        ...
        neg_infty = -1.0e9
        prev_attn = tf.zeros([1, 1, step, step])
        ...
        for m in range(self.n_layers):
            ...
            x_s_scores = tf.matmul(x_sq, x_sk, transpose_b=True)
            x_s_alphas = tf.nn.softmax(
                x_s_scores + prev_attn + attn_mask)
            prev_attn  = x_s_alphas
