...
tmp_seq_in  = np.zeros([batch_size, seq_encode], dtype=np.int32)
tmp_seq_dec = np.zeros([batch_size, seq_decode+1], dtype=np.int32)
...
while n_iter < maximum_iter:
    ...
    tmp_seq_dec[:, :] = PAD_token
    tmp_seq_dec[:, 0] = SOS_token
    ...
    for n_index in range(batch_size):
        ...
        in_noisy = np.random.choice(vocab_size, size=n_input)
        in_noise = [
            tmp_i_idx[x] if tmp_flag[x] == 1 else \
                in_noisy[x] for x in range(n_input)]
        del tmp_flag
        
        tmp_flag  = np.random.binomial(1, 0.1, size=n_output)
        out_noisy = np.random.choice(vocab_size, size=n_output)
        out_noise = [
            tmp_o_idx[x] if tmp_flag[x] == 0 else \
                out_noisy[x] for x in range(n_output)]
        del tmp_flag
        
        tmp_seq_dec[n_index, 1:n_decode] = out_noise
        tmp_seq_out[n_index, 1:n_decode] = tmp_o_idx
        
        tmp_seq_in[n_index, :n_input]  = in_noise
        tmp_seq_dec[n_index, n_decode] = EOS_token
        tmp_seq_out[n_index, n_decode] = EOS_token
    
    tmp_input  = tmp_seq_in
    tmp_decode = tmp_seq_dec[:, :-1]
