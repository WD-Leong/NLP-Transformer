...
class TransformerNetwork(torch.nn.Module):
    def __init__(
        ...
        self.p_decoder = torch.nn.Parameter(
            0.1 * torch.randn(self.hidden_size, self.dec_vocab_size))
    ...
    def transformer_encode(self, x, p_reg=1.00, p_keep=0.90):
        ...
        for m in range(self.n_layers):
            ...
            if self.attn_type == "add_attn":
                x_e_q = torch.unsqueeze(x_e_q, axis=3)
                x_e_k = torch.unsqueeze(x_e_k, axis=2)
                
                x_scores = torch.matmul(
                    tanh(x_e_q + x_e_k), self.v_e_s[m])
                x_alphas = softmax(torch.squeeze(x_scores, 4))
            else:
                x_scores = torch.matmul(
    ...
    def transformer_decode(
        ...
        for m in range(self.n_layers):
            ...
            if self.attn_type == "add_attn":
                x_d_q = torch.unsqueeze(x_d_q, axis=3)
                x_d_k = torch.unsqueeze(x_d_k, axis=2)
                
                x_self_scores = torch.matmul(
                    tanh(x_d_q + x_d_k), self.v_d_s[m])
                x_self_alphas = softmax(
                    torch.squeeze(x_self_scores, 4) + attn_mask)
            else:
                x_self_scores = torch.matmul(
            ...
            if self.attn_type == "add_attn":
                x_a_q = torch.unsqueeze(x_a_q, axis=3)
                x_a_k = torch.unsqueeze(x_a_k, axis=2)
                
                x_attn_scores = torch.matmul(
                    tanh(x_a_q + x_a_k), self.v_d_a[m])
                x_attn_alphas = \
                    softmax(torch.squeeze(x_attn_scores, 4))
            else:
                x_attn_scores = torch.matmul(
