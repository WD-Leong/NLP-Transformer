...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, v, k, q, mask=None):
        q_heads = self.split_heads(self.wq(q))
        k_heads = self.split_heads(self.wk(k))
        v_heads = self.split_heads(self.wv(v))
        
        attn_tuple = self.scaled_dot_product_attn(
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        # Apply Pre Layer Normalisation. #
        attn_self_norm = self.lnorm_2(attn_self_output)
        ffwd_self_output = self.ffwd_self(attn_self_norm)
        
        # Residual Connection. #
        res_output = tf.add(
            attn_self_output, ffwd_self_output)
        res_output = self.dropout_2(
