...
class DecoderLayer(tf.keras.layers.Layer):
    def __init__(
        ...
        self.lnorm_1 = tf.keras.layers.LayerNormalization(epsilon=1e-6)
        self.lnorm_2 = tf.keras.layers.LayerNormalization(epsilon=1e-6)
...
class Decoder(tf.keras.layers.Layer):
    def __init__(
        ...
        self.dec_layers = [DecoderLayer(
            d_model, n_heads, d_ffwd, rate) for x in range(n_layers)]
